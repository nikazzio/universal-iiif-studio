...
def render_settings_page(cm: Any) -> None:
    ...
    def _read_int(dotted: str, default: int) -> int:
        try:
            return int(cm.get_setting(dotted, default) or default)
        except (TypeError, ValueError):
            return default

    def _read_float(dotted: str, default: float) -> float:
        try:
    ...
    def _apply_and_save() -> None:
        ...
        # Back-compat: older configs may have seconds (e.g. 3) instead of ms.
        if 0 < toast_ms <= 60:
    ...
    _init_state("cfg_download_workers", _read_int("system.download_workers", 4))
    _init_state("cfg_ocr_concurrency", _read_int("system.ocr_concurrency", 1))
    _init_state("cfg_request_timeout", _read_int("system.request_timeout", 30))
    ...
    _init_state("cfg_items_per_page", _read_int("ui.items_per_page", 12))
    toast_ms_init = _read_int("ui.toast_duration", 3000)
    # Back-compat: if stored as seconds (<=60), convert to ms for the slider.
    ...
    _init_state("cfg_viewer_quality", _read_int("images.viewer_quality", 95))
