...
def render_main_canvas(doc_id, library, paths, stats=None, ocr_engine="openai", current_model="gpt-5"):
    ...
    has_pages = (stats and stats.get("pages")) or (meta and meta.get("pages")) or (paths.get("scans") and os.path.exists(paths["scans"]))
    if has_pages:
        thumb_source = {}
...
def render_history_sidebar(doc_id, library, current_p, current_data=None, current_text=""):
    ...
    with st.container(height=650):
        ...
        for i in range(len(history)):
            ...
            with st.container():
                st.markdown(f"<p style='margin-bottom:0px; font-size:0.95rem;'><b>{ts}</b>{v_label}{status_icon}</p><small style='color:#777; font-size:0.85rem;'>{icon} {eng}</small>", unsafe_allow_html=True)
                c1, c2 = st.columns([2, 1])
                c1.markdown(f"<span style='font-size:0.95rem;'><b>{chars} ch</b></span> <span style='color:{diff_color}; font-size:0.8rem; font-family:monospace;'>({diff_str})</span>", unsafe_allow_html=True)
