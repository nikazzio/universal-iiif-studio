...
def _render_header(current_page: int, total_pages: int, p_stat: dict | None) -> None:
    ...
    header_html = (
        ...
        f"{current_page} <span style='color: #444; font-weight: 300;'>/ {total_pages}</span>"
        "</h3>"
        "<span style='color: #888; font-size: 0.9rem;'>"
        f"📏 {p_stat['width']}×{p_stat['height']} px | 💾 {mb_size:.2f} MB |📍 ({progress_pct}%)"
        "</span>"
        "</div>"
    )
