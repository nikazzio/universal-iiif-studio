...
class TestProviders(unittest.TestCase):
    ...
    def test_01_vatican_resolver(self):
        ...
        d = IIIFDownloader(
            manifest_url, output_dir="tests/out", output_name="test_vat.pdf"
        )
    ...
    def run_partial_download(self, url, name):
        ...
        d = IIIFDownloader(url, output_dir="tests/out", output_name=f"{name}.pdf")
        ...
        if len(canvases) > 3:
            ...
            if "sequences" in d.manifest and d.manifest["sequences"]:
                d.manifest["sequences"][0]["canvases"] = d.manifest["sequences"][0][
                    "canvases"
                ][:3]

        d.run()

        # Verify images were downloaded (PDF is not auto-generated by default)
        self.assertTrue(os.path.isdir(d.scans_dir), "Scans directory was not created")
        jpgs = [
            f
            for f in os.listdir(d.scans_dir)
            if f.startswith("pag_") and f.endswith(".jpg")
        ]
        self.assertGreater(len(jpgs), 0, "No page images were downloaded")
        print(f"   (Downloaded {len(jpgs)} pages into: {d.scans_dir})")
