...
def _theme_preview_script(default_primary: str, default_accent: str) -> str:
    ...
    return (
        ...
        "state.primary=primary; state.accent=accent;"
        "state.saved={"
        ...
        "state.primary.value=state.saved.primary; state.accent.value=state.saved.accent;"
        ...
        "};"
        "applyTheme(st.saved.primary, st.saved.accent, st.root); st.dirty=false;"
        "}else{"
        "syncInputsToSaved(); applyTheme(st.saved.primary, st.saved.accent, st.root); st.dirty=false;"
        "}"
        "});"
